...
export function CategoryCard({ category }: { category: CategoryWithCount }) {
  return (
    ...
    >
      <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-creek-soft text-creek-dark transition-colors group-hover:bg-creek-dark group-hover:text-white">
      ...
      <ArrowUpRight
        className="h-5 w-5 shrink-0 text-ink-faint transition-colors group-hover:text-creek"
        aria-hidden="true"
      />
    </Link>
  );
