...
// Themed clay teardrop pin (a divIcon avoids Leaflet's bundler icon-path issues).
const pinIcon = L.divIcon({
  className: "",
  html: `<svg width="28" height="38" viewBox="0 0 28 38" xmlns="http://www.w3.org/2000/svg">
    <path d="M14 0C6.27 0 0 6.27 0 14c0 9.6 12.4 22.4 13 23a1.4 1.4 0 0 0 2 0c.6-.6 13-13.4 13-23C28 6.27 21.73 0 14 0z" fill="#bf522d" stroke="#fffdf9" stroke-width="2"/>
    ...
    >
      ...
      {points.map((p, i) => (
        <Marker key={p.slug ?? `${p.latitude},${p.longitude},${i}`} position={[p.latitude, p.longitude]} icon={pinIcon}>
          <Popup>
            {p.slug ? (
              <Link href={`/business/${p.slug}`} className="font-semibold text-[#9c3e1f]">
                {p.name}
              </Link>
            ) : (
